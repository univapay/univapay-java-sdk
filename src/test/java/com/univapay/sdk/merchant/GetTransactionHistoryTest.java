...
class GetTransactionHistoryTest extends GenericTest {
  ...
  @Test
  void shouldReadCorrectlyTransactionHistoryContainingCardPayments() throws Exception {
    ...
    assertNull(cardData.getGateway(), "This will be removed in the future so will return null");
  }

  @Test
  void shouldReadCorrectlyTransactionHistoryContainingConvenienceStorePayments() throws Exception {
    mockRRGenerator.GenerateMockRequestResponseJWT(
        "GET",
        "/stores/45facc11-efc8-4156-8ef3-e363a70a54c3/transaction_history",
        jwt,
    ...
    //noinspection deprecation
    assertNull(userData.getGateway(), "This will be removed in the future so will return null");
  }

  @Test
  void shouldReadCorrectlyTransactionHistoryContainingQrCpmPayments() throws Exception {
  ...
  @Test
  void shouldReadCorrectlyTransactionHistoryContainingQrMpmPayments() throws Exception {
    MockRRGenerator mockRRGenerator = new MockRRGenerator();
    mockRRGenerator.GenerateMockRequestResponseJWT(
        "GET",
        "/stores/45facc11-efc8-4156-8ef3-e363a70a54c3/transaction_history",
  ...
  @Test
  void shouldReadCorrectlyTransactionHistoryContainingOnlinePayments() throws Exception {
    MockRRGenerator mockRRGenerator = new MockRRGenerator();
    mockRRGenerator.GenerateMockRequestResponseJWT(
        "GET",
        "/stores/45facc11-efc8-4156-8ef3-e363a70a54c3/transaction_history",
  ...
  @Test
  void shouldReadCorrectlyTransactionHistoryContainingPaidyPayments() throws Exception {
    MockRRGenerator mockRRGenerator = new MockRRGenerator();
    mockRRGenerator.GenerateMockRequestResponseJWT(
        "GET",
        "/stores/45facc11-efc8-4156-8ef3-e363a70a54c3/transaction_history",
  ...
  @Test
  void shouldRequestAndReturnTransactionHistory() throws Exception {
    ...
    assertEquals("45facc11-efc8-4156-8ef3-e363a70a54c3", firstTransaction.getStoreId().toString());
    assertEquals(
        "e1771339-b989-4a43-99c1-5e35d8008426", firstTransaction.getResourceId().toString());
    assertEquals(
        "bae4fd94-aace-437c-bf63-8908008dff81", secondTransaction.getResourceId().toString());
    assertEquals(
        "6da25a85-754c-41db-97ec-c2e50f7591d5", thirdTransaction.getResourceId().toString());
    ...
    assertEquals("JPY", firstTransaction.getCurrency());
    assertEquals("CAD", secondTransaction.getCurrency());
    assertEquals("EUR", thirdTransaction.getCurrency());
    assertEquals(TransactionType.CHARGE, firstTransaction.getTransactionType());
    assertEquals(TransactionType.CHARGE, secondTransaction.getTransactionType());
    assertEquals(TransactionType.REFUND, thirdTransaction.getTransactionType());
    assertEquals(TransactionStatus.FAILED, firstTransaction.getStatus());
    assertEquals(TransactionStatus.FAILED, secondTransaction.getStatus());
    assertEquals(TransactionStatus.SUCCESSFUL, thirdTransaction.getStatus());
    ...
    assertEquals(ProcessingMode.TEST, firstTransaction.getMode());
    assertEquals(ProcessingMode.TEST, secondTransaction.getMode());
    assertEquals(ProcessingMode.TEST, thirdTransaction.getMode());
    ...
    assertThat(
        ...
        is("CARD HOLDER0"));
    assertNull(firstTransaction.getUserData().asCardTransactionData().getGateway());
    assertThat(firstTransaction.getUserData().getTransactionType(), is(TransactionType.CHARGE));
    assertThat(
        firstTransaction
            .getUserData()
            .asChargeUserData()
    ...
    assertThat(
        firstTransaction.getUserData().asChargeUserData().getRefunds().get(0).getReason(),
        is(RefundReason.DUPLICATE));
    assertThat(
        firstTransaction.getUserData().asChargeUserData().getRefunds().get(0).getAmount(),
        is(BigInteger.valueOf(9000)));
    assertThat(
        firstTransaction.getUserData().asChargeUserData().getRefunds().get(0).getCurrency(),
        is("JPY"));
    assertThat(
        firstTransaction.getUserData().asChargeUserData().getRefunds().get(0).getAmountFormatted(),
    ...
    assertThat(
        ...
        is("Konbini Customer"));
    assertThat(
        secondTransaction.getUserData().asKonbiniTransactionData().getConvenienceStore(),
        is(Konbini.LAWSON));
    assertNull(secondTransaction.getUserData().asKonbiniTransactionData().getGateway());
    assertThat(secondTransaction.getUserData().asChargeUserData().getRefunds().size(), is(0));
    assertThat(thirdTransaction.getPaymentType(), is(PaymentTypeName.PAIDY));
    assertThat(
        thirdTransaction.getUserData().asPaidyTransactionData().getCardholderEmailAddress(),
        is("cardholder_mail@univapay.com"));
    assertThat(
        thirdTransaction.getUserData().asPaidyTransactionData().getCardholderPhoneNumber(),
        is("810312345678"));

    assertThat(thirdTransaction.getUserData().getTransactionType(), is(TransactionType.REFUND));
    assertThat(
        thirdTransaction.getUserData().asRefundUserData().getRefundReason(),
        is(RefundReason.CUSTOMER_REQUEST));
    assertThat(response.getItems().get(3).getPaymentType(), is(PaymentTypeName.QR_SCAN));
    assertThat(
        response
            .getItems()
            .get(3)
    ...
    assertNull(response.getItems().get(3).getUserData().asQRScanTransactionData().getGateway());
    assertThat(
        firstTransaction.getUserData().asCardTransactionData().getCardBrand(),
        is(CardBrand.MASTERCARD));
    assertThat(response.getItems().get(4).getPaymentType(), is(PaymentTypeName.APPLE_PAY));
    assertThat(
        response.getItems().get(4).getUserData().asApplePayTransactionData().getCardBrand(),
        is(CardBrand.MASTERCARD));
    assertThat(
        response.getItems().get(4).getUserData().asApplePayTransactionData().getCardBrand(),
        is(CardBrand.MASTERCARD));
    assertThat(
        response.getItems().get(4).getUserData().asApplePayTransactionData().getCardHolderName(),
        is("CARD HOLDER4"));
    assertNull(response.getItems().get(4).getUserData().asApplePayTransactionData().getGateway());
  }

  @Test
  void shouldRequestAndReturnTransactionHistoryWithDateRange() throws Exception {
        ...
        .dispatch(
            new UnivapayCallback<PaginatedList<Transaction>>() {
              ...
              public void getResponse(PaginatedList<Transaction> response) {
                ...
                assertEquals("CAD", response.getItems().get(1).getCurrency());
                assertEquals("EUR", response.getItems().get(2).getCurrency());
                assertEquals(
                    TransactionType.CHARGE, response.getItems().get(0).getTransactionType());
                assertEquals(
                    TransactionType.CHARGE, response.getItems().get(1).getTransactionType());
                assertEquals(
                    TransactionType.REFUND, response.getItems().get(2).getTransactionType());
                assertEquals(TransactionStatus.FAILED, response.getItems().get(0).getStatus());
                assertEquals(TransactionStatus.FAILED, response.getItems().get(1).getStatus());
                assertEquals(TransactionStatus.SUCCESSFUL, response.getItems().get(2).getStatus());
                ...
                assertEquals(ProcessingMode.TEST, response.getItems().get(2).getMode());
                notifyCall();
              }
